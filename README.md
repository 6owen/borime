...
前 5 个候选的缓存缺失项会写入本地队列；停止输入约 800 毫秒后，Node.js sidecar 只取最新 5 个未命中候选并批量翻译。新输入出现时，正在进行的旧模型请求会在约一个轮询周期内取消；被取消批次的第一候选进入内存补偿队列，等输入空闲后补翻译，避免首选词永久漏掉。Rime 的按键线程从不等待网络。
