...
worker 看到候选后等待 800 ms，再次检查队列。如果期间仍有按键产生新候选，就放弃旧窗口并重新选择最新 5 条。模型请求发出后仍每 250 ms 检查队列；新输入会通过
...
代价是：尚处于 800 ms 防抖阶段、从未实际发起模型请求的短暂中间状态仍会被折叠。已经发起过请求的第一候选会在空闲时补偿；其余旧候选如有需要，重新输入并停顿即可再次触发。补偿队列只存在于 worker 内存中，重启 worker 会清空它。
